...
{
    public class NormalizeMinMaxMulticolumn
    {
        //https://learn.microsoft.com/en-us/dotnet/api/microsoft.ml.normalizationcatalog.normalizeminmax?view=ml-dotnet
        ...
        public static void Example()
        {
            ...
            var originalColumn = data.GetColumn<float[]>("Features").ToArray();
            var originalColumn2 = data.GetColumn<float[]>("Features2").ToArray();

            for (int i = 0; i < originalColumn.Length; i++)
                Console.WriteLine(string.Join(", ", originalColumn[i].Select(x => x
                .ToString("f4"))) + "\t\t" +
                string.Join(", ", originalColumn2[i].Select(x => x.ToString("f4"))));
